...
export default function AsciiFluid() {
  ...
  useEffect(() => {
    ...
    const renderFrame = (frame: number) => {
      ...
      // Fill interior with a stable “circuit” texture (symmetric), then overlay slow “activity”
      // via a few randomized blocks each tick (also symmetric).
      for (let y = iy0; y <= iy1; y++) {
        ...
        for (let x = ix0; x <= ix1; x++) {
          ...
          if (inCore) {
            ...
            // “Activity” blocks: a few squares change each tick.
    ...
    // Slow, consistent tick rate (simple + stable).
    let frame = 0;
    renderFrame(frame);
